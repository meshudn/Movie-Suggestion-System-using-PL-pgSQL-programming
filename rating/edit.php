...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    pg_query($db_handle, "SELECT edit_rating('{$rating_id}','{$rating}','{$user_name}','{$film_title}','{$release_year}')");
}
        ...
        <div class="row justify-content-md-center">
            <div class="col-md-auto">
                ...
                <form action="" method='POST'>
                    <?php
                        $result = pg_query($db_handle, "SELECT * from user_rating where rating_id = " .$rating_id);
                        while ($row = pg_fetch_assoc($result)) {
                            ?>
                    <input type="text" name="film_title" value='<?php echo $row['film_title']; ?>' hidden>
                    <input type="text" name="rating_id" value='<?php echo $row['rating_id']; ?>' hidden>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" name="rating" value='<?php echo $row['rating']; ?>'>
                    </div>
