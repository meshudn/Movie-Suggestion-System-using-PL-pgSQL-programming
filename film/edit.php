...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    pg_query($db_handle, "SELECT edit_film('{$film_id}','{$film_title}','{$release_year}','{$production_country}', '{$age_limit}')");
}
        ...
        <div class="row justify-content-md-center">
            <div class="col-md-auto">
                ...
                <form action="" method='POST'>
                    <?php
                        $result = pg_query($db_handle, "SELECT * from films where films.film_id = " .$film_id);
                        while ($row = pg_fetch_assoc($result)) {
                            ?>
                    <input type="text" name="film_id" value='<?php echo $row['film_id']; ?>' hidden>

                    <div class="mb-3">
                        <label for="film_title">Film Title</label>
                        <input type="text" class="form-control" name="film_title"
                            value='<?php echo $row['film_title']; ?>' >
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <label for="production_country">Production Country</label>
                        <input type="text" class="form-control" name="production_country"
                            value='<?php echo $row['production_country']; ?>'>
                    </div>
                    <div class="mb-3">
                        <label for="age_limit">Age Limit</label>
                        <input type="text" class="form-control" name="age_limit"
                            value='<?php echo $row['age_limit']; ?>' >
                    </div>
