...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $release_year = '';
    $query_result_2 = pg_query($db_handle, "SELECT release_year from films WHERE film_title = '{$film_title}'");
    while ($row_1 = pg_fetch_assoc($query_result_2)) {
            $release_year = $row_1['release_year'];
    }

    $role_id = time();
    pg_query($db_handle, "SELECT add_role('{$role_id}','{$role_title}','{$person_name}','{$film_title}','{$release_year}')");
        ...
        <div class="row justify-content-md-center">
            <div class="col-md-auto">
                ...
                <form action="" method='POST'>
                    ...
                    <div class="mb-3">
                        <label class="form-label" for="role_title">Role</label>
                        <input type="text" class="form-control" name="role_title">
                    </div>

                    <div class="mb-3">
                        <label class="form-label" for="film_title">Sub Film (Year)</label>
                        <select class="form-select" name="film_title">
                            <?php 
                                $query_result = pg_query($db_handle, "SELECT * FROM films
